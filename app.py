...
tab1, tab2 = st.tabs(["Gibbs Triangle with Phase Regions", "Simple Gibbs Triangle (Light Color Gradient)"])
...
# --- TAB 2: Simple Gibbs Triangle (Light Color Gradient) ---
with tab2:
    st.header("Simple Gibbs Triangle (Light Color Gradient)")
    ...
    if C2 < 0:
        ...
        # Generate a grid of points inside the triangle for the gradient
        resolution = 80  # Higher = smoother gradient, lower = faster
        points = []
        colors = []
        for a in range(resolution + 1):
            for b in range(resolution + 1 - a):
                ...
                # Light RGB color proportional to (A, B, C), blend with white
                base_color = np.array([aa/100, bb/100, cc/100])
                light_color = 0.6 * base_color + 0.4 * np.ones(3)  # Blend with white for lightness
                points.append((x, y))
        ...
        # Plot the light color gradient
        ax.scatter(points[:,0], points[:,1], c=colors, s=10, marker='s', linewidths=0, alpha=1, zorder=1)
        ...
        ax.plot(x2, y2, 'ro', markersize=12, zorder=4)
        ax.text(x2, y2 + 0.03, f"({A2:.1f}, {B2:.1f}, {C2:.1f})", ha='center', fontsize=12, fontweight='bold', color='black', zorder=5)
