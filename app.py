...
tab1, tab2 = st.tabs(["Gibbs Triangle with Phase Regions", "Simple Gibbs Triangle (No Phase Lines)"])
...
# --- TAB 2: Simple Gibbs Triangle (No Phase Lines) ---
with tab2:
    st.header("Simple Gibbs Triangle (No Phase Lines)")
    ...
    if C2 < 0:
        ...
        ax.plot(x2, y2, 'ro', markersize=12)
        ax.text(x2, y2 + 0.03, f"({A2:.1f}, {B2:.1f}, {C2:.1f})", ha='center', fontsize=12, fontweight='bold', color='black')
        ...
        st.download_button(
            label="Download diagram as PNG",
            data=buf.getvalue(),
            file_name="simple_ternary_diagram.png",
            mime="image/png"
        )
