...
B = st.slider("Component B (%)", 0, 100, step=5, value=35)
C = st.slider("Component C (%)", 0, 100, step=5, value=15)

total = A + B + C

if total > 100:
    st.error(f"Invalid input: A + B + C = {total}%. The sum must not exceed 100%.")
elif total < 100:
    st.warning(f"Note: A + B + C = {total}%. The sum should be exactly 100% for a valid ternary diagram.")
else:
